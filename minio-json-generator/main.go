...
func generateRandomData(r *rand.Rand) *message {
	...
	randomValue := r.Intn(len(ID))
	_ = fmt.Sprintf("Key-%s", ID[randomValue]) // key is generated but not used, keeping your original logic
	
	return &message{
		User:  names[r.Intn(len(names))],
		Car:   cars[r.Intn(len(cars))],
		Color: color[r.Intn(len(color))],
	}
}
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			default:
				objectName := fmt.Sprintf("%d.json", currentNumber)
				...
				if err != nil {
					...
					if atomic.LoadInt64(&uploadedCount)%10 == 0 {
						log.Printf("Uploaded %d objects so far... Current: %s - User: %s, Car: %s, Color: %s", 
							atomic.LoadInt64(&uploadedCount), objectName, data.User, data.Car, data.Color)
					}
				}
