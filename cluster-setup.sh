...
cleanup(){
  log "CLEANUP ..."
  sudo sed -i '/minio-ui.kind.cluster kibana.kind.cluster grafana.kind.cluster alertmanager.kind.cluster single.kind.cluster agent.kind.cluster vl.kind.cluster"$/d' /etc/hosts
